...
const postLocs = {
    ...
    grain: gl.getUniformLocation(postProgram, "u_grain") // NEW
};
...
// NEW: Dynamic Name Generator based on the UI Seed Slider!
function generateNames(seed) {
    ...
    for(let i = 0; i < MAX_GLOBAL_REGIONS; i++) {
        ...
        // Adds a fun 10% chance for a Kingdom to have a Roman Numeral suffix
...
const inputs = {
    ...
    name_seed: document.getElementById('name_seed'), // NEW BINDING
...
// Old File Import Listener updated to use the function above
...
// Build the UI list of saved maps
function renderProjects() {
    // Ask the backend for ONLY this user's maps
...
function render() {
    ...
    // --- CULLING LOGIC: Find the closest regions to the camera! ---
    let visibleRegions = [];
    let activeCount = Math.floor(state.region_count || 100);
    let aspect = canvas.width / canvas.height;
